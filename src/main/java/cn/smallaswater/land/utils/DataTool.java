...
public class DataTool {
    ...
    public static LinkedList<Player> getAroundPlayers(Position position, int size) {
        ...
        for (Player player : position.level.getPlayers().values()) {
            if (player.x < position.x + size &&
                    player.x > position.x - size &&
                    player.z < position.z + size &&
                    player.z > position.z - size &&
                    player.y < position.y + size &&
                    player.y > position.y - size) {
                if (!explodePlayer.contains(player)) {
                    explodePlayer.add(player);
                }
            }
        }
